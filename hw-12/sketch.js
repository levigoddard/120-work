...
function setup() {
    ...
    for (let i = 0; i < num_of_dudes; i++) {
        dudes.push( new MarchingDude(init_x, init_y) );
      init_x += 100;
      if (init_x > width) {
      init_x = 60;
      init_y += 100;
...
class MarchingDude {
    ...
    ballCheck(otherDudes, myId) ; {
        for (let n = 0; n < otherDudes.length; n++) {
            if (n != myId) {
                let d = dist(this.posX, this.posY, otherDudes[n].posX, otherDudes[n].posY);
                let combinedR = this.rad + otherDudes[n].rad;
